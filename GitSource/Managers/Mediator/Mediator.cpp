...
string Mediator::getTeamWithHighestScore()
{
	int max = 0;
	string name = "NONE";

	for (auto& score : _teamsPoints)
	{
		if (max < score.second)
		{
			max = score.second;
	...
	for (auto& score : _teamsPoints)
	{
		if (max == score.second && name != score.first)
		{
			name = "NONE";
		}
	}
...
void Mediator::eraseAbility(int id)
{
	...
	TimeManager::getTimer(to_string(id) + _activeAbilities[id])->resetTimer();
