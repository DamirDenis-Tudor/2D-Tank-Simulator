...
bool SpriteComponent::isOnCamera()
{
	if (_dest->x + _dest->w  < -256 ||
		_dest->y + _dest->h  < -256 ||
		_dest->x  > RendererManager::_width + 256  ||
		_dest->y  > RendererManager::_heigth + 256 )
	{
		/*
...
SpriteComponent::SpriteComponent(const char* source, int sourceWidth , int sourceHeight , int destWidth , int destHeight )
{
	_src = new SDL_Rect;
	_dest = new SDL_Rect;
