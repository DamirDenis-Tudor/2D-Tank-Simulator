...
AnimationComponent::AnimationComponent(string _name, Vector2T<int> position, float angle, float timer)
	:_position(position), animationSpeed(timer)
{
	...
	*/
	_frames = size(AssetsStorage::_effects[_name]);
	for (int i = 0; i < _frames; i++)
	{
		SpriteComponent* sprite = new SpriteComponent(AssetsStorage::_effects[_name][i]);
...
{
	_anim[_frameID]->setCameraPosition(_position - CameraManager::offset - _anim[_frameID]->getWidth() / 2);
	_anim[_frameID]->draw();
}
