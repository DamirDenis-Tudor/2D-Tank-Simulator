...
Tank::Tank(map<string, SpriteComponent*>& parts, Behavior*& behavior, TankAttributes*& attributes, string type, string color)
	:_parts(parts), _behavior(behavior), _attributes(attributes), _name(type), _teamColor(color)
{
	disable();
	...
	*/
	InfoManager::addInfo(to_string(_id) + "HP", new TextComponent(RED, 32));

	//seteaza un chenar in jurul textului de viata
	//InfoManager::setDrawingBorder(to_string(_id) + "HP");

	/*
	* informatie text despre abilitatea activa
	*/
	InfoManager::addInfo(to_string(_id) + "ability", new TextComponent(GOLD, 32));
...
{
	/*
	* daca avem tinta pe un tank inamic sau tank prieten
	* aceasta isi schimba culoare
	* acest comportament este valabil doar daca avem
	* jucator extern
	*/
	
	//aparent nu am mai facut asta
	...
	if (timerOn)
	{
		...
		*/
		InfoManager::setCameraPosition(to_string(_id) + "reload",
			Vector2T<int>{ _position._x, _position._y - AssetsStorage::_tileDim - InfoManager::getDimension(to_string(_id) + "reload")._y / 2 - 64 } +
	...
	if (!timerOn && _behavior->isLaunchingBullet())
	{
		...
		{
			SoundManager::playSoundEfect("fire" , 45);
		}
		else if(_parts["body"]->isOnCamera())
		{
			SoundManager::playSoundEfect("fire1", 25);
		}
		...
		*/
		...
		SpecialObjectsManager::addBullet(new Bullet(_name, _attributes->_impactAnim, _attributes->_bulletDamage, _position + circumference + _parts["cannon"]->getWidth() / 2, _parts["cannon"]->_angle, _id));
...
void Tank::launchMine()
{
	...
	{
		if (_parts["body"]->isOnCamera())
		{
			SoundManager::playSoundEfect("releaseMine", 30);
		}
...
void Tank::checkOfHits()
{
	...
	if (Mediator::getHealth(_id) <= 0)
	{
		...
		*/
		...
		if (CameraManager::getFocusId() == _id)
		{
			SoundManager::playSoundEfect("tankExplosion" , 70);
		}
		else if(_parts["body"]->isOnCamera())
		{
			SoundManager::playSoundEfect("tankExplosion", 30);
		}
		temporaryDisable();
	}
...
void Tank::checkOfHealing()
{
	...
	*/
	...
	Mediator::modifyHealth(_id, 8);

	if (CameraManager::getFocusId() == _id && Mediator::getHealth(_id) != 100)
	{
		SoundManager::playSoundEfect("heal", 30);
	}
...
void Tank::checkOfAbilities()
{
	...
	*/
	InfoManager::setCameraPosition(to_string(_id) + "ability",
		Vector2T<int>{ _position._x + AssetsStorage::_tileDim - InfoManager::getDimension(to_string(_id) + "ability")._x / 2, _position._y - AssetsStorage::_tileDim } - CameraManager::offset);
