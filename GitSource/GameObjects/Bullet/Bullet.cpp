	...
	: _position(position), _tankId(tankId)  , _angle(angle), _damage(damage) , _impactAnim(impactAnim)
{
	...
	_sprite->setCameraPosition(position - _sprite->getWidth() / 2 - CameraManager::offset);
...
void Bullet::update()
{
	...
	*/
	...
	potentialPos._x += _velocity._x * SDL_cos((_sprite->getAngle() - 90) * M_PI / 180) * TimeManager::getDeltaTime();
	potentialPos._y += _velocity._y * SDL_sin((_sprite->getAngle() - 90) * M_PI / 180) * TimeManager::getDeltaTime();
	...
	*/
	...
	_sprite->setCameraPosition(_position - CameraManager::offset - _sprite->getWidth() / 2);
}
